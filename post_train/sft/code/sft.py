...
一、SFT 原理（面向实现）
1) 使用“指令-回答”监督数据，让模型学习目标输出分布。
2) 训练目标通常是自回归语言模型的 token-level 交叉熵损失。
3) 通过最小化训练 loss，使模型在给定提示下更稳定地产生期望回答。
4) 结合验证 loss、学习率和梯度范数，可判断收敛与稳定性。

二、代码框架（从入口到结果）
1) `parse_args`：读取训练与可视化参数。
2) `detect_device_and_precision`：自动选择设备与混合精度。
3) `build_train_config`：构建最小可用 SFT 配置。
...
def resolve_output_dir(base_dir: Path, output_dir: str) -> Path:
    """解析输出目录：相对路径按 sft 目录解析，绝对路径原样使用。"""
    out = Path(output_dir)
...
def resolve_factory_dir(code_dir: Path, module_dir: Path) -> Path:
    """定位 LLaMA-Factory 目录：优先 code 下，其次 sft 根目录。"""
    candidates = [
...
def find_trainer_state(checkpoints_dir: Path) -> Path | None:
    ...
    # 优先读取 checkpoints 根目录下的 trainer_state.json。
...
def main() -> None:
    ...
    factory_dir = resolve_factory_dir(code_dir=code_dir, module_dir=module_dir)
    config_path = layout["output"] / "train_sft_auto.json"
