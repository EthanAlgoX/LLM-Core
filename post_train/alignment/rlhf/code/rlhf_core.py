...
CORE_READING_ORDER = [
    ...
    ("run_core_rlhf_flow", "RLHF 主流程：环境 -> 配置 -> 训练 -> 导出"),
...
def print_core_learning_guide() -> None:
    """打印新手学习主线，帮助快速定位关键函数。"""
    print("=== RLHF 新手学习主线（建议按顺序阅读）===", flush=True)
...
def _export_placeholder_artifacts(metrics_dir: Path, keys: list[str], note: str) -> None:
    ...
    try:
        ...
        ax.axis("off")
        ax.text(0.02, 0.6, "RLHF Toy Placeholder", fontsize=16, weight="bold")
...
def run_core_rlhf_flow(args: argparse.Namespace, layout: dict[str, Path], factory_dir: Path) -> Path:
    """RLHF 主流程（新手重点）：环境检测 -> 配置生成 -> 训练 -> 导出结果。"""
    ...
    print("[步骤 3/5] 启动 RLHF 训练", flush=True)
    train_ok = True
    try:
        run_train(factory_dir=factory_dir, config_path=config_path)
    except Exception as exc:
        train_ok = False
        print(f"[WARN] RLHF training failed, fallback to placeholder artifacts: {exc}", flush=True)
...
def main() -> None:
    ...
    metrics_dir = run_core_rlhf_flow(args=args, layout=layout, factory_dir=factory_dir)
