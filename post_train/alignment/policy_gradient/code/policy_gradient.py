...
新人阅读顺序（建议）：
1) 先看 `main` 里的训练参数段：搞清策略模型、奖励模型与训练步长。
2) 再看 `main`：理解训练流程是如何落盘与执行的。
3) 最后看 `export_policy_gradient_visualization`：学会看关键指标曲线。
...
def export_policy_gradient_visualization(checkpoints_dir: Path, output_dir: Path) -> Path:
    ...
    # 新手优先关注：
    # reward/kl/objective_kl 体现策略更新质量，loss/lr 体现优化稳定性。
    keys = [
        "step",
        "epoch",
        "loss",
...
def main() -> None:
    ...
    # 新手提示：终端步骤号（1~5）可直接映射到本函数每个步骤注释。
