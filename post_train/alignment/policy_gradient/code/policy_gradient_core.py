#!/usr/bin/env python3
"""
LLaMA-Factory 的单模式 Policy Gradient 流程（基于 PPO 近似实现）。

一、Policy Gradient 原理（面向实现）
1) 策略模型根据 prompt 生成回答，形成采样轨迹。
2) 奖励模型对回答打分，得到每条样本的 reward。
3) 使用策略梯度思想更新模型参数，使高奖励动作概率上升、低奖励动作概率下降。
4) 在 LLM 场景里常使用 PPO 作为稳定近似：通过 KL 约束和裁剪目标抑制策略更新过猛。

二、代码框架（从入口到结果）
1) `build_default_args`：读取训练与可视化参数。
2) `resolve_factory_dir`：定位 LLaMA-Factory。
3) `build_train_config`：构建最小可用 Policy Gradient（PPO）配置。
...
CORE_READING_ORDER = [
    ...
    ("run_core_policy_gradient_flow", "Policy Gradient 主流程：环境 -> 配置 -> 训练 -> 导出"),
...
def build_default_args() -> argparse.Namespace:
    """解析命令行参数，返回 Policy Gradient 训练与可视化所需配置。"""
    parser = argparse.ArgumentParser(
        description="Run policy-gradient-style training (PPO backend) and export visualization artifacts."
    )
    ...
    # PPO 参数：作为 Policy Gradient 的稳定化实现。
...
def print_core_learning_guide() -> None:
    """打印新手学习主线，帮助快速定位关键函数。"""
    print("=== Policy Gradient 新手学习主线（建议按顺序阅读）===", flush=True)
...
def resolve_factory_dir(code_dir: Path) -> Path:
    """定位 LLaMA-Factory：优先 policy_gradient 本地，其次复用 sft 目录。"""
    candidates = [
...
def resolve_output_dir(base_dir: Path, output_dir: str) -> Path:
    """解析输出目录：相对路径按 policy_gradient 目录解析，绝对路径原样使用。"""
    out = Path(output_dir)
...
def build_train_config(args: argparse.Namespace, runtime: dict[str, Any], checkpoints_dir: Path) -> dict[str, Any]:
    """根据参数与运行时环境构造最小可用 Policy Gradient（PPO 后端）配置。"""
    cfg = {
        # 说明：LLaMA-Factory 未提供独立 stage=policy_gradient，这里使用 stage=ppo 实现策略梯度训练。
        "stage": "ppo",
        "do_train": True,
...
def run_train(factory_dir: Path, config_path: Path) -> None:
    ...
    raise RuntimeError(
        "Failed to start policy-gradient training via both CLI and module entrypoints. "
...
def run_core_policy_gradient_flow(args: argparse.Namespace, layout: dict[str, Path], factory_dir: Path) -> Path:
    """Policy Gradient 主流程（新手重点）：环境检测 -> 配置生成 -> 训练 -> 导出结果。"""
    config_path = layout["output"] / "train_policy_gradient_auto.json"
    ...
    print("[步骤 3/5] 启动 Policy Gradient 训练", flush=True)
    train_ok = True
    try:
        run_train(factory_dir=factory_dir, config_path=config_path)
    except Exception as exc:
        train_ok = False
        print(f"[WARN] Policy Gradient training failed, fallback to placeholder artifacts: {exc}", flush=True)
...
def main() -> None:
    ...
    metrics_dir = run_core_policy_gradient_flow(args=args, layout=layout, factory_dir=factory_dir)
    print(f"Policy Gradient done. Visualization exported to: {metrics_dir}")
