#!/usr/bin/env python3
"""
LLaMA-Factory 的单模式 Actor-Critic 流程（基于 PPO 近似实现）。

一、Actor-Critic 原理（面向实现）
1) Actor（策略网络）负责根据 prompt 采样回答。
2) Critic（价值网络）负责估计价值，降低策略梯度方差。
3) 奖励模型给出 reward 信号，驱动 Actor 朝高奖励方向更新。
4) 在 LLM 训练里常使用 PPO 作为稳定化的 Actor-Critic 近似实现。

二、代码框架（从入口到结果）
1) `build_default_args`：读取训练与可视化参数。
2) `resolve_factory_dir`：定位 LLaMA-Factory。
3) `build_train_config`：构建最小可用 Actor-Critic 配置。
...
CORE_READING_ORDER = [
    ...
    ("run_core_actor_critic_flow", "Actor-Critic 主流程：环境 -> 配置 -> 训练 -> 导出"),
...
def build_default_args() -> argparse.Namespace:
    """解析命令行参数，返回 Actor-Critic 训练与可视化所需配置。"""
    parser = argparse.ArgumentParser(
        description="Run actor-critic-style training (PPO backend) and export visualization artifacts."
    )

    # 模型与数据：策略模型 + 奖励模型是核心输入。
...
def print_core_learning_guide() -> None:
    """打印新手学习主线，帮助快速定位关键函数。"""
    print("=== Actor-Critic 新手学习主线（建议按顺序阅读）===", flush=True)
...
def resolve_factory_dir(code_dir: Path) -> Path:
    """定位 LLaMA-Factory：优先 actor_critic 本地，其次复用 sft 目录。"""
    candidates = [
...
def resolve_output_dir(base_dir: Path, output_dir: str) -> Path:
    """解析输出目录：相对路径按 actor_critic 目录解析，绝对路径原样使用。"""
    out = Path(output_dir)
...
def build_train_config(args: argparse.Namespace, runtime: dict[str, Any], checkpoints_dir: Path) -> dict[str, Any]:
    """根据参数与运行时环境构造最小可用 Actor-Critic（PPO 后端）配置。"""
    cfg = {
        # 说明：LLaMA-Factory 没有独立 stage=actor_critic，这里使用 stage=ppo 实现。
        "stage": "ppo",
        "do_train": True,
...
def run_train(factory_dir: Path, config_path: Path) -> None:
    ...
    raise RuntimeError(
        "Failed to start actor-critic training via both CLI and module entrypoints. "
...
def _export_placeholder_artifacts(metrics_dir: Path, keys: list[str], note: str) -> None:
    ...
    try:
        ...
        ax.axis("off")
        ax.text(0.02, 0.6, "Actor-Critic Toy Placeholder", fontsize=16, weight="bold")
...
def export_learning_artifacts(checkpoints_dir: Path, output_dir: Path, ema_alpha: float) -> Path:
    ...
    x, y = _extract_series(rows, ["loss", "ppo/loss/policy"])
    axes[0, 0].plot(x, y, marker="o", alpha=0.45, label="actor_loss(raw)")
    if y:
        axes[0, 0].plot(x, _ema(y, ema_alpha), linewidth=2, label=f"actor_loss(ema={ema_alpha})")
    axes[0, 0].set_title("Actor Loss")
    axes[0, 0].set_xlabel("step")
    axes[0, 0].grid(True, alpha=0.3)
    axes[0, 0].legend()

    x, y = _extract_series(rows, ["value_loss", "ppo/loss/value"])
    axes[0, 1].plot(x, y, marker="o", alpha=0.45, color="#ff7f0e", label="critic_loss(raw)")
    if y:
        axes[0, 1].plot(x, _ema(y, ema_alpha), linewidth=2, color="#d62728", label=f"critic_loss(ema={ema_alpha})")
    ...
    if y:
        axes[1, 0].plot(x, _ema(y, ema_alpha), linewidth=2, label=f"reward(ema={ema_alpha})")
    ...
    summary = {
        ...
        "final_actor_loss": next(
            (r["loss"] for r in reversed(rows) if r["loss"] is not None),
            next((r["ppo/loss/policy"] for r in reversed(rows) if r["ppo/loss/policy"] is not None), None),
        ),
        "final_critic_loss": next(
            (r["value_loss"] for r in reversed(rows) if r["value_loss"] is not None),
            next((r["ppo/loss/value"] for r in reversed(rows) if r["ppo/loss/value"] is not None), None),
        ),
...
def run_core_actor_critic_flow(args: argparse.Namespace, layout: dict[str, Path], factory_dir: Path) -> Path:
    """Actor-Critic 主流程（新手重点）：环境检测 -> 配置生成 -> 训练 -> 导出结果。"""
    config_path = layout["output"] / "train_actor_critic_auto.json"
    ...
    print("[步骤 3/5] 启动 Actor-Critic 训练", flush=True)
    train_ok = True
    try:
        run_train(factory_dir=factory_dir, config_path=config_path)
    except Exception as exc:
        train_ok = False
        print(f"[WARN] Actor-Critic training failed, fallback to placeholder artifacts: {exc}", flush=True)
...
def main() -> None:
    ...
    metrics_dir = run_core_actor_critic_flow(args=args, layout=layout, factory_dir=factory_dir)
    print(f"Actor-Critic done. Visualization exported to: {metrics_dir}")
