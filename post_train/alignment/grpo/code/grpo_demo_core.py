...
CORE_READING_ORDER = [
    ...
    ("run_core_grpo_flow", "GRPO 主流程：数据 -> 配置 -> 训练 -> 导出"),
    ("build_trainer", "按 TRL 版本构建训练器"),
    ("export_training_metrics", "导出 JSON/CSV/可视化曲线"),
    ("main", "程序入口"),
]
...
def build_default_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-size", type=int, default=4, help="合成训练样本数量。")

    parser.add_argument("--learning-rate", type=float, default=5e-7)
    parser.add_argument("--num-train-epochs", type=float, default=0.05)
...
def print_core_learning_guide() -> None:
    """打印新手学习主线，帮助快速定位关键函数。"""
    print("=== GRPO 新手学习主线（建议按顺序阅读）===", flush=True)
...
def run_core_grpo_flow(args: argparse.Namespace, layout: dict[str, Path]) -> tuple[Path, dict[str, Any]]:
    """GRPO 主流程（新手重点）：数据构建 -> 配置生成 -> 训练 -> 导出结果。"""
    print("[步骤 1/5] 固定随机种子并准备数据", flush=True)
    set_seed(args.seed)
    train_dataset = build_dataset(train_size=args.train_size, seed=args.seed)

    print("[步骤 2/5] 组装奖励函数与训练配置", flush=True)
    ...
    return metrics_dir, train_output.metrics
